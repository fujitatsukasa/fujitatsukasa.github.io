...
<script>
  ...
  // 次/前ボタン（クリックイベントは、下記インジケータークリックに合わせ、左右ボタンは削除する）
  // ※ここでは背景なしのシンプルな矢印ボタンを手動で用意するため、JSはインジケーターと自動再生のみ
  ...
  document.querySelectorAll('.carousel-image').forEach((img, index) => {
    img.addEventListener('click', () => {
      ...
      modalInfo.textContent = 'クリックして拡大表示 (画像 ' + (index + 1) + ' / ' + slideCount + ')';
    });
  });
